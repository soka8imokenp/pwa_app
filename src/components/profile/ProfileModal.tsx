...
export const ProfileModal: React.FC<ProfileModalProps> = ({
  ...
  const handleLinkGoogle = async () => {
    ...
    try {
      ...
      if (google?.accounts?.oauth2 && !Capacitor.isNativePlatform()) {
          ...
          callback: async (tokenResponse: any) => {
            if (tokenResponse?.access_token) {
              try {
                ...
                if (res.user) {
                  onUpdateProfile({ ...currentUser, ...res.user });
                  setEmail(res.user.email);
                  setFeedback(
                    language === 'uz'
                      ? 'Google hisobi muvaffaqiyatli bogʻlandi!'
                      : language === 'ru'
                ...
                setFeedback(err?.message || 'Linking Google account failed');
              } finally {
      ...
      setFeedback(e?.message || 'Failed to initiate Google link');
      setIsLinkingGoogle(false);
    }
  };
          ...
          {activeTab === 'overview' ? (
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4 animate-in fade-in duration-150">
              ...
              <div className="p-4 bg-white border-[1.75px] border-[#24201D] rounded-2xl shadow-[2px_2px_0px_#24201D] space-y-3">
                ...
                <div className="pt-2">
                  <div className="p-3 bg-[#FAF8F5] border-[1.75px] border-[#24201D] rounded-xl flex items-center justify-between gap-2.5 shadow-2xs">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs font-black text-[#24201D] truncate">
                            {language === 'uz' ? 'Google Hisobi' : language === 'ru' ? 'Аккаунт Google' : 'Google Account'}
                          </span>
                          {email.endsWith('@gmail.com') && (
                            <span className="text-[9px] font-black text-[#2D503C] bg-[#DDE8DE] px-1.5 py-0.2 rounded border border-[#24201D]/20">
                              ✓ {language === 'uz' ? 'Ulangan' : language === 'ru' ? 'Подключен' : 'Connected'}
                            </span>
                          )}
                        </div>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-white hover:bg-[#F4F0EA] border-[1.5px] border-[#24201D] text-xs font-black text-[#24201D] shadow-[1.5px_1.5px_0px_#24201D] active:translate-y-0.5 transition-all cursor-pointer disabled:opacity-50 shrink-0"
                    >
                      {isLinkingGoogle
                        ? '...'
                        : language === 'uz'
